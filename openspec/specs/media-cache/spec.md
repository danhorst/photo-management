...
Name and track each source volume in the skip cache, and let the user list and
clear cached volumes, so the cache can be managed by name rather than opaque id
and stale entries from reformatted cards can be removed.
